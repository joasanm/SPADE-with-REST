...
#requests that receive Rest are stored in a dictionary
#peticiones = {}
#ID = 0
...
class restRequest(spade.Behaviour.OneShotBehaviour):
    def _process(self):
        ...
        result = ""
        try:
            r = requests.get("http://"+rmh+"/patients", data = content[1], timeout = 3) #COMPLETAR/COMPROBAR
...
class hospitalDonant(spade.Behaviour.Behaviour):
    def _process(self):
        msg = self._receive(block=True)
        aid = msg.getSender().getName()
...
#request form: [request state, request used, empty string for information to send]
@app.route('/<string:hospital>/region', methods=['GET'])        #COMPLETAR
def consult(hospital):
    a = ""
...
if __name__ == "__main__":
    ...
    if spadeHost == "" or restHost == "" or rgName == "" or len(remoteHosts) == 0 or len(hospitals) == 0:
        print "Require next arguments: -s Spade_host -r rest_host -rg region -u [remote_hosts] -h [hospitals]"
        sys.exit(0)
    if len(hospitals) != len(remoteHosts):
        print "The number of remote hosts and hospitals must be equal"
        sys.exit(0)
    ...
    for i in hospital_list:
        print "Hospital: " + i["hospital"] + " -> " + i["remoteHost"]
    ...
    t = spade.Behaviour.MessageTemplate(aclt)
    rg.addBehaviour(hospitalDonant(), t)
